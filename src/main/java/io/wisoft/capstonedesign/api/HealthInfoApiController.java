...
import io.wisoft.capstonedesign.service.HealthInfoService;
...
//        return new Result(infoDtoList);
    ...
    /* 특정 병과의 건강 정보 목록 조회 */
    @GetMapping("/api/health-infos/department/{department}")
    public Result healthInfosByDepartment(@PathVariable("department") String department) {

        List<HealthInfoDto> infoDtoList = healthInfoService.findAllByDept(department)
                .stream().map(HealthInfoDto::new)
                .collect(Collectors.toList());
